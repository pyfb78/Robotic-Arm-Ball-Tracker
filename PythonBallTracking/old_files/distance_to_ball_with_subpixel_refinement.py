...
def main():
    ...
    while True:
        ...
        min_area = 75  # Adjusted minimum area threshold
        ...
        if len(filtered_contours) > 0:
            ...
            # Refine the circle center using subpixel accuracy
            gray_frame = cv2.cvtColor(undistorted_frame, cv2.COLOR_BGR2GRAY)
            corners_to_refine = np.float32([[[x, y]]])
            criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 10, 0.01)
            cv2.cornerSubPix(gray_frame, corners_to_refine, (5, 5), (-1, -1), criteria)
            refined_center = corners_to_refine[0][0]

            center = np.array(refined_center, dtype=np.float32)
            radius = int(radius)

            if radius >= 10:  # Adjusted minimum enclosing circle radius threshold
                # Draw a circle around the ball
                cv2.circle(frame, (int(center[0]), int(center[1])), radius, (0, 255, 0), 2)
